...
// MODIFICATION : On enlève evaluateAnswerWithGemini car elle est maintenant sur le serveur.
import { generateRemediation, generateTextToSpeech } from './services/geminiService'; 
...
  // --- DEBUT DE LA MODIFICATION CRITIQUE ---
  const handleSubmitAnswer = async () => {
    if (!userResponse.trim()) return;
    setLoading(true);

    const currentQ = QUESTIONS[currentQIndex];
    // Déclaration de la variable pour stocker l'évaluation
    let evaluation: { status: 'correct' | 'partial' | 'wrong', feedback: string }; 
    
    try {
        // NOUVEAU CODE : Appel sécurisé à la Netlify Function (Endpoint)
        const endpoint = '/.netlify/functions/evaluate';
        ...
        const response = await fetch(endpoint, {
            ...
            body: JSON.stringify({ 
                // Données nécessaires à l'évaluation par le serveur
                question: { 
                    text: currentQ.text,
        ...
        if (!response.ok) {
            ...
            try {
                ...
                errorMsg = errorBody.feedback || errorMsg;
            } catch {}
            throw new Error(errorMsg);
        }

        // Récupérer le résultat du serveur
        evaluation = await response.json();
        
    } catch (error) {
        console.error("Validation Error:", error);
        // En cas d'échec de la requête (problème réseau ou serveur), on affiche un message d'erreur à l'utilisateur
        evaluation = { status: 'wrong', feedback: `Analyse non disponible. (${error.message || 'Erreur inconnue'})` };
    }

    setLoading(false);
    
    // Logic based on rules (Reste du code INCHANGÉ)
...
    }
  };
  // --- FIN DE LA MODIFICATION CRITIQUE ---
